# Regole e Contesto del Progetto (v2.3.2)

Questo file definisce le linee guida e lo stato di salvataggio del progetto per garantire la coerenza con la versione **2.3.2**.

## Stato di Riferimento (v2.3.2)

1. **Gestione Versioni**:
   - La versione attuale di riferimento è **2.3.2**.
   - Qualsiasi modifica futura richiede l'avanzamento della versione (es. `2.3.3` o successive) in `app.js` (`const APP_VERSION = "X.Y.Z";`) e in `index.html` (header).
...
2. **Bypass della Cache (Cache-Busting)**:
   ...
   - Esempio: `<link rel="stylesheet" href="style.css?v=2.3.2">` e `<script src="app.js?v=2.3.2"></script>`.
